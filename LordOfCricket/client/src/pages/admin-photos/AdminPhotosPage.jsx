...
import { useAdminPhotos } from '../../hooks/useAdminPhotos.js'
  ...
  return (
    <div className="min-h-screen bg-emerald-950 px-6 py-12">
      <div className="mx-auto flex max-w-3xl flex-col gap-10">
        <h1 className="text-3xl font-bold text-white">Manage Ground Photos</h1>
        ...
        >
          ...
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium text-emerald-100/70">Title (optional)</label>
            <input
              type="text"
              value={title}
        ...
        <div className="flex flex-col gap-4">
          <h2 className="text-lg font-semibold text-white">
            Current Photos {loading ? '' : `(${photos.length})`}
          </h2>

          {loading ? (
            <p className="text-emerald-100/60">Loading…</p>
          ) : photos.length === 0 ? (
            <p className="text-emerald-100/60">No photos yet — upload one above.</p>
          ) : (
